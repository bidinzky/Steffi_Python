...
class SensorValue:
    def __init__(self, value: float, time: str):
        self.value = value
        self.time = time
...
class Datastore:
    def __init__(self, max_size: int):
        self.data: Deque[SensorValue] = collections.deque(maxlen=max_size)

    def addValue(self, sv: SensorValue):
        self.data.append(sv)
...
class ControlSystem(Datastore):
    def __init__(self, max_size: int, min_threshold: float = -math.inf, max_threshold: float = math.inf):
